...
import day4.AbstractFeeCalculator;
import day4.CarCalculator;
import day4.BikeCalculator;
import day4.TruckCalculator;

public class ArrayListObjectSortExample {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        String choice = "yes";
        ArrayList<AbstractFeeCalculator> sessionHistoryCopy = new ArrayList<>();
        List<AbstractFeeCalculator> sessionHistory = new ArrayList<>();
...
//                	sessionHistory.add(calculator.copy());
                	break;
            ...
            sessionHistory.add(calculator);
            sessionHistoryCopy.add(calculator.copy());
            
            sessionHistory.sort(Comparator.comparingDouble(AbstractFeeCalculator::calculateTotalFee));
        ...
        System.out.print("Do you want to print session history with sorted fees(yes/no)? ");
        boolean printHistory = sc.next().equalsIgnoreCase("yes");

        if (printHistory) {
            System.out.println("\n--- Parking Session History ---");
            for (AbstractFeeCalculator session : sessionHistory) {
                System.out.println("Vehicle: " + session.getVehicleName());
